...
function process(mavenFiles: PathOrFileDescriptor[], result: Map<string, ResultType>): Map<string, ResultType> {
    ...
    result.set('cmd_build', result.get('cmd') + ' clean verify -DskipTests -B -q');
    result.set('cmd_test_build', result.get('cmd') + ' clean verify -B -q');
    result.set('cmd_update_deps', result.get('cmd') + ' versions:use-latest-versions -B -q -DgenerateBackupPoms=false -Dmaven.version.ignore=".+-(alpha|beta|RC[0-9]+|M[0-9]+).*"');
